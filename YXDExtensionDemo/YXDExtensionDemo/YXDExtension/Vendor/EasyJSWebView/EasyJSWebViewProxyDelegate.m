...
- (void)injectJavascript:(UIWebView *)webView {
    ...
    for(id key in self.javascriptInterfaces) {
        ...
        NSArray<NSString *> *methodNameArray = [self methodNameArrayWithInterface:interface];
        
        for (int i = 0; i < methodNameArray.count; i++){
            [injection appendString:@"\""];
            [injection appendString:methodNameArray[i]];
            [injection appendString:@"\""];
...
- (NSArray<NSString *> *)methodNameArrayWithInterface:(NSObject *)interface {
...
//    while (cls && (cls != [UIViewController class])) {
        ...
        for (int i = 0; i < mc; i++) {
            ...
            if (![arr containsObject:methodName]) {
